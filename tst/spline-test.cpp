...
//     std::cout << "{" << std::to_string(p.pose.x * std::cos(1)) << ", " << std::to_string(p.pose.x * std::sin(1)) << '\n';
//   }
// }
...
TEST(impose_limits_test, cap_velocity_1d) {
  ...
  std::vector<PathPosition> parameterized_path = spline.parameterize(test_path_1d);
...
TEST(impose_limits_test, cap_velocity_2d) {
  ...
  std::vector<PathPosition> parameterized_path = spline.parameterize(test_path_2d);
  for (auto p : parameterized_path) {
    DEBUG_PRINT(p.to_string());
    EXPECT_LE(std::abs(p.vel), max_vel);
    EXPECT_LE(std::abs(p.accel), max_accel);
