...
class VisData(Structure):
    ...
    def __repr__(self):
        return f"VisData: {{time: {self.time}, rx: {self.rx}, ry: {self.ry}, ryaw: {self.ryaw}, rv: {self.rv}, ra: {self.ra}, rj: {self.rj}}}"
...
def main():
    ...
    clib.compute_path.restype = POINTER(VisData)
    ...
    for i in range(150):
        time.append(visdata[i].time)
        rx.append(visdata[i].rx)
        ry.append(visdata[i].ry)
        ryaw.append(visdata[i].ryaw)
        rv.append(visdata[i].rv)
        ra.append(visdata[i].ra)
        rj.append(visdata[i].rj)
