...
VisData compute_path(double sx,
                     ...
                     double track_width,
                     double dt) {
  auto constraints = Constraints(max_vel, max_accel, max_jerk, 2.0);
  ...
  std::vector<SplineGenerator::GeneratedPoint> path = spline.gen_raw_path(ControlVector(s, sv, sa), ControlVector(g, gv, ga));
  VisData out;
  out.size = path.size();
